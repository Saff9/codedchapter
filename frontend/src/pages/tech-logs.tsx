...
import { Hash, Search, AlertCircle, Cpu, Terminal } from "lucide-react";
...
export default function TechLogs() {
  ...
  useEffect(() => {
    updateMetaTags({
      title: "Tech Chapters | Coded Chapter",
      description: "Dev logs, code retrospective breakdowns, and engineering details.",
      canonicalUrl: "https://codedchapter.vercel.app/tech",
    });
  }, []);
  ...
  return (
    <div className="relative min-h-screen">
      {/* Ambient glows */}
      <div className="pointer-events-none fixed -top-40 -right-40 w-[500px] h-[500px] rounded-full bg-primary/6 blur-[160px]" />
      <div className="pointer-events-none fixed -bottom-40 -left-40 w-[400px] h-[400px] rounded-full bg-secondary/5 blur-[140px]" />
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="mb-10">
              <div className="flex items-center gap-2 mb-3">
                <span className="text-[11px] font-mono text-primary bg-primary/10 border border-primary/20 px-2.5 py-1 rounded-full">
                  // tech_dev_logs
                </span>
                {/* Live pill */}
              ...
              <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
                <div>
                  <h1 className="text-3xl md:text-4xl font-bold tracking-tight" style={{ fontFamily: "Space Grotesk, sans-serif" }}>
                    {tagParam ? (
                      <>Tech posts tagged <span className="text-transparent bg-clip-text bg-gradient-to-r from-primary to-amber-300">#{tagParam}</span></>
                    ) : (
                      <>Tech <span className="text-transparent bg-clip-text bg-gradient-to-r from-primary to-amber-300">Chapters</span></>
                    )}
                  </h1>
                  <p className="text-sm text-muted-foreground mt-1.5">
                    Dev logs, code retrospectives, and engineering breakdowns.
                  </p>
                </div>
            ...
            {error ? (
              <div className="py-14 px-8 text-center border border-destructive/20 rounded-2xl bg-destructive/5 flex flex-col items-center gap-3">
                ...
                <p className="text-sm font-semibold text-destructive">Failed to load tech chapters</p>
              ...
              <div className="py-24 flex flex-col items-center text-center border border-dashed border-border/40 rounded-2xl bg-card/20">
                <div className="w-12 h-12 rounded-xl bg-muted/50 border border-border/40 flex items-center justify-center mb-4">
                  <Cpu className="w-5 h-5 text-muted-foreground/50" />
                </div>
                <p className="text-sm font-medium text-muted-foreground">No tech posts found</p>
                <p className="text-xs text-muted-foreground/60 mt-1">
                  {tagParam || searchQuery ? "Try a different search or tag." : "Check back soon for new chapters."}
                </p>
            ...
            <div className="sticky top-24 space-y-1">
              ...
              <div className="mt-6 p-3 rounded-xl border border-border/40 bg-card/50">
                ...
                <p className="text-[10px] text-muted-foreground/60 mt-1 leading-relaxed">
                  All chapters are pulled directly from Substack.
                </p>
              </div>
            </div>
          </aside>
