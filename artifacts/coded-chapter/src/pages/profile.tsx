...
export default function ProfilePage() {
  ...
  const authorPosts = allPosts?.filter(p => p.authorName === profile?.displayName || p.authorId === profile?.userId) ?? [];
  ...
  useEffect(() => {
    ...
    api.getProfile(username).then((p: any) => {
      setProfile(p);
      setLoading(false);
      api.listDoubts().then((d: any[]) => setDoubts(d.filter(x => x.authorId === p.userId)));
  ...
  if (!profile) {
    return (
      <div className="container mx-auto px-6 py-24 max-w-3xl text-center space-y-4">
        ...
        <Link href="/"><button className="px-4 py-2 border border-border rounded-lg text-sm hover:bg-muted transition-colors">Go Home</button></Link>
      </div>
    );
  }
    ...
    <div className="container mx-auto px-6 lg:px-8 py-12 md:py-16 max-w-3xl">
      ...
      >
        ...
        <div className="px-6 pb-6">
          ...
          <div className="flex gap-6 mt-5 pt-5 border-t border-border/40">
            {[
              { icon: <BookOpen className="w-3.5 h-3.5" />, label: "Posts", val: authorPosts.length },
              { icon: <HelpCircle className="w-3.5 h-3.5" />, label: "Doubts", val: doubts.length },
              { icon: <HelpCircle className="w-3.5 h-3.5" />, label: "Answered", val: doubts.filter(d => d.isResolved).length },
            ].map(({ icon, label, val }) => (
      ...
      <div className="flex gap-1 mb-6 bg-muted/40 p-1 rounded-lg w-fit">
        {(["posts", "doubts"] as const).map(t => (
          ...
          >
            {t === "posts" ? `Posts (${authorPosts.length})` : `Doubts (${doubts.length})`}
          </button>
        ))}
      </div>
