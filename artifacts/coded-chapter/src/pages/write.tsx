...
import {
  ...
  FileText, AlignLeft, Tag, Image as ImageIcon
} from "lucide-react";
...
export default function WritePage() {
  ...
  const { data: existing } = useGetPost(editId ?? 0, { query: { enabled: !!editId, queryKey: getGetPostQueryKey(editId ?? 0) } });
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      const body = { title, excerpt, content, tags, coverImage: coverImage || undefined };
        ...
        <div className="flex-1 container mx-auto px-6 lg:px-8 py-8 max-w-4xl">
          {preview ? (
            ...
            >
              ...
              <div
                className="prose prose-sm prose-invert max-w-none prose-p:leading-[1.8] prose-code:text-amber-400 prose-code:bg-amber-400/10 prose-code:px-1.5 prose-code:rounded prose-code:before:content-none prose-code:after:content-none"
                style={{ whiteSpace: "pre-wrap" }}
              >
                {content || <span className="text-muted-foreground italic">Nothing written yet…</span>}
              </div>
            </motion.div>
          ) : (
            <motion.div
